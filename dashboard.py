...
st.title("KQL Query Generator")

# requests which contain "CreateReturn" or "Import Proforma" since last 12 hours and summarize total of every 30 mins
table_tab = None
chart_tab = None
...
queries = [
    ...
    "exceptions | summarize NumberOfExceptions=count() by type | order by NumberOfExceptions desc | top 10 by NumberOfExceptions",
...
# Screen 2: Main Functionality
    ...
    st.write("Azure App Insights Visualizer Tool")
    st.info('Azure App Insights Retriever')
    with st.sidebar:
    ...
    requests which contain "CreateReturn" or "Import Proforma" since last 12 hours and summarize total of every 30 mins
    '''
    sample_query = sample_q.strip()
    # st.write(sample_query)
    query = st.text_input("Ask your request here", value=sample_query)
...
# Screen 3: How To Use
    ...
    # ... your code for the how to use screen ...
    st.markdown("# How To Use")
    st.markdown("*(Insert your instructions here)*")

    # Back button to Screen 2
    #if st.button("Back"):
    #    st.session_state.screen = 2
    st.button("Back", on_click=on_back_select, key="back_button_how_to_use")
